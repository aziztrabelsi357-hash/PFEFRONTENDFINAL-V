...
export default function AIPlantDetection() {
  ...
  const handleDetection = async () => {
    ...
    setLoading(true);
    try {
      // Use query param for model (matches your backend)
      const res = await axios.post(`http://localhost:8080/api/detect?model=${modelType}`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      const data = Array.isArray(res.data) ? res.data : [];
      // Transform to a consistent prediction shape with corners
      ...
      setHighlightId(null);
      toast.success(`Detected ${preds.length} object(s) with model "${modelType}".`);
    } catch (e) {
      console.error(e);
  ...
  // Filtered preds based on UI threshold (visual only)
  ...
  // Base draw (fit canvas to container)
  ...
  // Draw base when preview changes
  useEffect(() => {
    drawBase();
  }, [drawBase]);

  // Redraw when data/UI changes
  useEffect(() => {
    drawDetections();
  }, [drawDetections]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-green-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <FaLeaf className="text-green-600 text-6xl mx-auto mb-3" />
          <h1 className="text-5xl font-extrabold text-green-800">🌱 AI Plant Detection</h1>
          <p className="text-gray-600">Detect plant diseases or classify leaves (best.pt / best_leaf.pt)</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          ...
          <div className="xl:col-span-1 bg-white rounded-2xl shadow-xl p-6 border border-green-100">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              {!usingCamera ? (
                <button
                  onClick={startCamera}
                  className="bg-green-100 hover:bg-green-200 text-green-700 py-2 rounded-lg flex items-center justify-center gap-2"
                >
                  <FaCamera /> Use Camera
                </button>
              ) : (
              ...
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50"
              >
            ...
            <div className="mt-6 space-y-4">
              <div>
                ...
                >
                  <option value="plant">🌿 Plant Disease Model</option>
                  <option value="leaf">🍃 Leaf Model</option>
                </select>
              </div>
          ...
          <div className="xl:col-span-2 bg-white rounded-2xl shadow-xl p-6 border border-green-100">
            ...
            <div
              ref={containerRef}
              className="relative w-full bg-gray-50 rounded-lg min-h-[420px] overflow-hidden flex items-center justify-center"
            >
              {usingCamera ? (
                <div className="relative w-full">
                  ...
                  <button
                    onClick={capturePhoto}
                    className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-full shadow"
                  >
                    Capture Photo
                  </button>
                </div>
                ...
                <div className="text-gray-400 text-center">
                  <FaLeaf className="text-6xl mx-auto mb-2" />
                  No image selected
                </div>
              )}
              ...
              {detectionResult && (
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      navigator.clipboard
                        .writeText(JSON.stringify({ ...detectionResult, predictions: filteredPreds }, null, 2))
                        .then(() => toast.success("Filtered results copied!"))
                    }
            ...
            {!detectionResult ? (
              ...
              <AnimatePresence>
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  ...
                  <div className="rounded-lg p-4 border bg-green-50 border-green-200">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium">
                        {filteredPreds.length} objects detected (≥ {confidenceThreshold}%)
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <b>{Object.keys(classCounts).length}</b>
                      </div>
                      <div>
                        <span className="text-gray-500">Avg conf:</span>{" "}
                        <b>{(avgConf * 100).toFixed(1)}%</b>
                      </div>
                      <div className="col-span-2">
                        <span className="text-gray-500">Top:</span>{" "}
                        <b>{maxLabel || "—"}</b> {maxConf ? `(${(maxConf * 100).toFixed(1)}%)` : ""}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Detection list (click to highlight) */}
                  <div className="space-y-2">
                    ...
                    {!filteredPreds.length && (
                      <p className="text-gray-400 text-sm">No detections above current threshold.</p>
                    )}
                  </div>
        ...
        <motion.div
          ...
          className="mt-8 bg-white rounded-2xl shadow-xl p-6 border border-green-100"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div className="bg-green-50 rounded-lg p-3 border border-green-200">
              ...
              <div className="text-green-700">{modelType === "plant" ? "best.pt (plant disease)" : "best_leaf.pt (leaf)"}</div>
            </div>
            <div className="bg-green-50 rounded-lg p-3 border border-green-200">
              <div className="font-medium text-green-800">Framework</div>
              <div className="text-green-700">Ultralytics YOLOv8</div>
            </div>
            <div className="bg-green-50 rounded-lg p-3 border border-green-200">
              <div className="font-medium text-green-800">Backend</div>
              <div className="text-green-700">Spring Boot + Python</div>
            </div>
            <div className="bg-green-50 rounded-lg p-3 border border-green-200">
              <div className="font-medium text-green-800">Endpoint</div>
              <div className="text-green-700">POST /api/detect?model={modelType}</div>
            </div>
          </div>
        </motion.div>
      </div>
